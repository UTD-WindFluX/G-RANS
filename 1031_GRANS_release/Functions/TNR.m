function TNR(fontsize_fig)
    ...
    for ID_obj=1:length(objs)
        if strcmp(objs(ID_obj).Type,'uimenu')==0
            ...
            l=[];
            try l=get(objs(ID_obj),'XLabel');catch err;end
            if ~isempty(l);objs(ID_obj).TickLabelInterpreter='latex';end
    ...
    obj=findall(gcf,'type','annotation');
    
    for ID_obj=1:length(obj)
        c=obj(ID_obj).Children;
        for ID_c=1:length(c)
            try
                if length(c(ID_c).Text.String)==3 && strcmp(c(ID_c).Text.String(1),'(') && strcmp(c(ID_c).Text.String(3),')')
                else
